...
```tsx
import { useSipManager, useSessionMethods } from 'react-sip-kit';

function DialPad() {
  const { status } = useSipManager();
...
⚠️ **Important:**
`useSipManager` re-renders only when lines are **added or removed**, not when the internal data of a line changes.
...
- Use `useSipManager` for high-level SIP state (status, lines list).
