...
function App({ configKey }: { configKey: string }) {
  const { dialByNumber } = RtcConnection.getHybridSessionMethodsBy({ configKey }); // all session methods like dial, answer, toggle video, toggle share-screen and so on
...
const SipLine = memo(({ lineKey }: { lineKey: LineType['lineKey'] }) => {
  const {
    ...
    toggleShareScreen,
    recordSession,
  } = RtcConnection.getHybridSessionMethodsBy({ lineKey });
